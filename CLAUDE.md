...
- **試材清單 / 樣品清單**:樣品以 `material_id` 連回試材,兩邊的詳細卡可互相跳轉;細節見下一節
...
### 試材與樣品的關聯
- 資料模型沒有變:樣品文件自己存一份 `species`、`name_zh`、`source`、`acquired_date`、`part`(從試材**複製**過來,不是參照),另外用 `material_id` 記來源試材。其他模組(GC 匯出對話框、圖譜綁定、Ctrl+K、備份)都直接讀樣品上的這些欄位,所以不能改成只存連結
- 試材詳細卡用 `smpItems.filter(s=>s.material_id===item.id)` 反查「由這個試材製備的樣品」,並有「以此試材新增樣品」(`startSmpFromMaterial`);樣品詳細卡有「來源試材」可跳回去。樣品快照更新時要一併 `renderMatTable()`,因為試材表格有「已製備樣品」計數
- 樣品表單選了來源試材就帶入並**鎖住**中文名稱、學名、來源、取得日期(`smpLockedFields`),要改得先按「解除連結」。**部位不鎖**:同一個試材(例如全果)可能只取果皮或種子來萃取。**要鎖哪些欄位看試材本身有沒有填,不看表單現值**(`lockSmpMaterialFields`):試材的中文名稱是選填、樣品卻必填,使用者自己補的值之後重開還要能改;依表單現值判斷的話,重開時就會把使用者自己打的字也鎖住。重選試材時,試材沒填的欄位只清掉「上一個試材帶進來的」值,使用者自己打的保留
- 連結的試材後來被刪掉時,`populateSmpSelects(currentMaterialId)` 會補一個「(試材已刪除)」選項,否則 select 對不到值,一存檔 `material_id` 就默默變成 null

### 樣品表單(四個步驟:試材、萃取、重量與萃取率、照片)
- 一筆樣品是**分好幾天填完**的(第一天鮮重與含水量取樣,隔天烘乾後的乾重,再過幾天才有萃取物重量),所以允許含水量只填一部分先存;萃取率框會列出「還缺」哪幾項,清單上有琥珀色的待補標籤(`smpPendingTags`:含水量只填了一部分,或已有絕乾重卻沒有萃取物重量。刻意只挑這兩種沒有疑義的情況,免得對本來就不打算填的樣品一直提醒)
- **絕乾重有兩種來源**,由 `s_dry_mode`(hidden input,`moisture` / `manual`)切換。含水量模式下絕乾重是唯讀的,隨時依「試材鮮重 × (1 − 濕基含水率)」重算(`recomputeSmpDry`);手動模式下絕乾重自己填,含水量區只有在還有內容時才顯示(附一行「只留作紀錄」的說明),因為藏起來的值一樣會被驗證、被存進去,唯讀的人也要看得到原本量測的數值與照片
- **`s_dry_mode` 不寫進 Firestore**,只存在表單與草稿裡。開啟既有樣品或舊版草稿時由 `inferSmpDryMode` 從數值推斷:**只有「重算後 `toFixed(3)` 的結果和存著的絕乾重一模一樣」才算含水量模式**,其餘一律當手動。不要改成容差比對:手動填的 1082.1627 會落在容差內被判成換算模式,然後被改寫成 1082.163。這是為了保證「只是打開再存檔」不會動到使用者原本的絕乾重,改這段邏輯前先想清楚這件事
- 同理,`smpStoredRate`:匯入的舊資料可能只有萃取率、沒有任何重量。絕乾重與萃取物重量都還空著時,`updateExtractionRate` 保留原本的萃取率,不會因為使用者動了鮮重或切了一下模式就清成空白(`openSmpForm` 設定,新增與清除時歸零)
- 表單狀態一律靠 `syncSmpFormState()` 一次對齊(分離部、溶劑或萃取物、含水量、絕乾重、萃取率、鎖定、編號),`syncSmpLocks()` 是唯一決定欄位 disabled 的地方(唯讀模式、試材鎖定、日期未知三件事一起算;以前分散設定,後面的迴圈會把「日期未知」時該停用的日期欄重新打開)
- 編號規則沒變,**格式不可以改**(已經印成 QR 貼在瓶子上),編輯既有樣品也不重算。`nextSeqForPrefix` 要求前綴之後整段都是數字:粗萃物的前綴同時是分離部編號(`…_MeE_HexF_03`)的開頭,只用 `startsWith` 比對會把分離部的序號算進粗萃物(踩過的坑)

### 表單共通的地雷(藥品、試材、樣品三個表單同一套寫法)
- **草稿計時器**:`debounce()` 回傳的函式有 `cancel()` 與 `flush()`。關閉表單時編輯模式要 `cancel()`、新增模式要 `flush()`;儲存成功後先 `cancel()` 再 `clearDraft()`。以前沒有這兩步,700ms 的計時器會在表單關閉、`editingId` 被設回 null 之後才觸發,把剛編輯的既有資料整張存成「新增草稿」,下次按新增就冒出上一筆的內容(踩過的坑)
- **儲存鎖**:用「第幾次開啟表單」的計數(`smpFormSession` / `smpSavingSession`,試材是 `mat…`)擋同一張表單重複送出,只停用儲存、刪除、清除三顆鈕;**取消與 ✕ 不能鎖**,離線時 Firestore 的 `set()` 要等連線回來才會結束,鎖住就關不掉表單。表單上的值在第一個 await 之前一次抓齊,儲存完成時只有畫面上還是同一次開啟的表單才收尾(關表單、清草稿);使用者中途去開了別筆,就不要動那張表單。建立者欄位要從函式開頭抓到的 `existingItem` 取
- 「複製並編輯」與「以此試材新增樣品」開的是預先填好的新增表單,手上有草稿時先 `confirmReplaceSmpDraft()` 問過再取代,不要把兩份內容混在一起
- 表格列的刪除鈕與縮圖一律 `createElement` / `addEventListener` 綁定,不要用 inline `onclick` 拼字串;寫進 `innerHTML` 的資料欄位一律過 `escapeHtml`
...
- **匯出正式報告**(iframe 的「正式報告」鈕,送 `{gcExportReport, rows, svg, title, name, date}`,主系統 `gcExportReport` / `grxBuildParts`):依實驗室既有的成分分析報告版面,A4 四邊 2 cm;第一頁「精油基本資料」與「氣相層析圖譜條件」兩張格線表(0.5pt),層析圖(本工具的 SVG 轉 PNG,寬 16.3 cm)放在條件表最後一列;分頁後是「Peak # / Compound / Retention time / KI / rKI / Relative area / Identification」三線表(不標色)。欄寬是依實際字串長度重新分配過的(日期格 4.5 cm、儀器與管柱值 10pt、三線表左右內距 60 twips),改欄寬前先確認這些字串仍各佔一行。對話框可從樣品清單帶入植物名稱/學名/來源/蒸餾方法/蒸餾日期/蒸餾者/採集日期/收率;層析條件記在 localStorage(`gc-report-cond`),預設值是實驗室的 Agilent 6850/5973 + HP-5MS 條件。OOXML 封裝骨架與期刊格式共用 `gdxPackage(body)`
- **手動積分表**(峰表上方可收合的面板,`parseTabulate` / `manualPeaks` / `tabulateText`):格式就是 MSD ChemStation「Tabulate → Copy」的內容(Peak #、Ret Time、Type、Width、Area、Start Time、End Time,第八欄 Height 選填;ChemStation 複製出來的第八欄是空的,「填入自動積分結果」會填峰頂扣基線的峰高;Tab 或空白分隔,標題列略過)。套用後該樣品 `s.manualOn=true`,`activePeaks()` 改回傳表內的峰,自動積分與 rtMin / minPct 篩選都不再作用(刪峰 `s.dropped` 仍有效);Area(%)、KI、圖上編號、Word / CSV 匯出、存入圖譜全部跟著走。Area 留空或填 `-` 的列,用 Start–End 兩端連成的直線基線重新積分;相鄰兩列 End / Start 相接(≤1.5 個取樣點)視為連峰,共用一條基線、交界處垂直切開。面積單位比照 ChemStation(abundance × 0.1 秒,即 abundance·min × 600,`CS_AREA`),用實際檔案對過大峰比值 594–601
...
- **圖上標籤與插圖編號錨在原始訊號的峰頂 `hy`,不是扣基線後的 `h`**(自動與手動模式都是,`integrate` / `manualPeaks` 都會記 `hy`)。手動表的 Start 若落在峰的側翼(例如 Start 只比峰頂早 0.018 min 的大峰),Start–End 直線基線從側翼高處起算,`h` 只剩峰頂的兩成,以前標籤就埋進峰裡(踩過的坑);`h` 仍保留 ChemStation 定義的峰高,供手動表的 Height 欄使用
...
- `tools/serve.ps1 [-Port 8765]`:不需要 node 或 python 的本機靜態伺服器(PowerShell 的 HttpListener),開 `http://localhost:8765/` 預覽。Google 登入需要 http 來源,直接雙擊開 `file://` 不行

改完 `index.html` 至少要跑一次 `check-syntax.ps1`。機器上沒有 node 時,改用 `serve.ps1` 開起來看瀏覽器 console 有沒有語法錯誤,並實際操作改到的流程。
